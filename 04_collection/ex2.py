...
result = [i**2 for i in range(1, 11)]       #함수 호출을 안해도 됨
print(result)
...
result = ['짝' if i % 2 == 0 else '홀' for i in range(1, 11)]       # if + else인 경우 (삼항연산자) 조건문을 앞으로 보냄

# 각 이름의 길이로 이루어진 리스트 만들기
names = ["pororo", "crong", "poby", "eddy"]
#[6, 5, 4, 4]
result = [len(names[i]) for i in range(0, 4)]
print(result)
...
result = [sum(scores[i][j] for i in range(3))/3 for j in range(3)]
